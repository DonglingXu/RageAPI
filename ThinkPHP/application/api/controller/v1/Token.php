...
use think\Request;
use app\api\controller\ResultData;
...
class Token
{
    ...
    {
        //参数验证
        $validate = new \app\common\validate\Token;
        ...
        $userInfo = ManagerService::FindManager(input('mobile'),input('password'));
        if (empty($userInfo)) self::returnMsg(422,'账号或密码错误');
        try {
    ...
    {
        $cache_refresh_token = Cache::store('redis')->get(self::$refreshAccessTokenPrefix.$appid);  //查看刷新token是否存在
    ...
    public static function checkParams($params = [])
    {
        ...
        // 使用Redis缓存
        //Cache::store('redis')->set('token_appid','tp5restfultest');
        // 正确的姿势

        if($params['appid'] !== Cache::store('redis')->get('token_appid')){
    ...
    {
        return Cache::store('redis')->get(self::$refreshAccessTokenPrefix.$appid) ? Cache::get(self::$refreshAccessTokenPrefix.$appid) : self::buildAccessToken();
    }
    ...
    {
        //存储accessToken
        cache::store('redis')->set(self::$accessTokenPrefix . $accessToken, $accessTokenInfo, self::$expires);
    }
    ...
    {
        //存储RefreshToken
        cache::store('redis')->set(self::$refreshAccessTokenPrefix.$appid,$refresh_token,self::$refreshExpires);
    }
}
