...
class Oauth
{
    ...
    public static function getClient()
    {
        ...
        try {
            ...
            if (empty($authorization)) {
                $authorization = Request::param('Authorization');
            }
    ...
    {
        $getCacheAccessToken = Cache::store('redis')->get(self::$accessTokenPrefix . $data['access_token']);  //获取缓存access_token
        if(!$getCacheAccessToken){
            return self::returnMsg(401,'fail',"access_token不存在或为空".$data['access_token']);
        }
